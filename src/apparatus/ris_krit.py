...
class Wall:
    def __init__(self, start_coords, end_coords, ris_feasible, orientation, wall_normal):
...
class RisController(VisibilityGraphCtrl):
    ...
    def populate_walls(self, polygons=True):
...
if __name__ == '__main__':
    results_folder_no_ris = os.path.join(os.getcwd(), "data_save_krit\\no_ris\\")
    results_folder_single_ris = os.path.join(os.getcwd(), "data_save_krit\\single_ris\\")
    results_folder_double_ris = os.path.join(os.getcwd(), "data_save_krit\\double_ris\\")
    sim_ctrl = SimulationController()
    ris_ctrl = RisController(sim_ctrl)
    ris_ctrl.set_source_and_dest([645, 1125], None)
...
# MAXIMUM_PL = 80  # 85, 100, 111 for 200, 100, 50 mbps
