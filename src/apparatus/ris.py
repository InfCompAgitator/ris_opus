...
class RisController(VisibilityGraphCtrl):
    ...
    def get_ris_visibility(self, ris_obj):
        return set(visible_vertices(Point(ris_obj.x[0],ris_obj.y[0]), self.vis_graph.graph) +
                   visible_vertices(Point(ris_obj.x[1],ris_obj.y[1]), self.vis_graph.graph))
    ...
    def save_rechable_pts_for_ns(self, folder_name=None):
        ...
        for i in range(1, N_HOPS_MAX):
            ...
            np.save(folder_name+f'\\n_hops_equal_{i}.npy', points)
