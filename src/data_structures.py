...
class Coords3d:
    ...
    def in_boundary(self, min_coords, max_coords):
        return min_coords.x <= self.x <= max_coords.x and min_coords.y <= self.y <= max_coords.y \
...
@dataclass
class Obstacle:
    __slots__ = ["id", "height", "vertices", "walls_normal"]
    id: int
    height: float
    vertices: list
    walls_normal: list
