...
from src.visibility_tools.visibility_graph import Point, VisGraph, Edge, visible_vertices
from src.visibility_tools.visibility_graph_ctrl import VisibilityGraphCtrl
from copy import deepcopy
from data_structures import Coords3d
...
class WwrfSimulator(SimulationController, VisibilityGraphCtrl):
    def __init__(self):
        super().__init__()
        ...
        self.walls = []
        self.vertices_list = []
        self.obstacles_objects = self.user_model.obstacles_objects
        self.populate_walls()
    ...
    def build_graph(self):
        super().build_graph()
        self.graph_backup = deepcopy(self.vis_graph.graph)
    ...
    def reload_graph_from_backup(self):
        self.vis_graph.graph = deepcopy(self.graph_backup)

    def add_edges_to_visibility_graph(self, point_1, point_2, ris):
        if point_2 in self.vis_graph.graph.get_adjacent_points(point_1):
            return
        _edge = Edge(point_1, point_2)
        _edge.ris_hop = ris
        self.vis_graph.graph.add_edge(_edge)
    ...
    def get_obstacles_segments(self, flatten=True):
        segs = self.obstacles_objects.get_total_segments()[0]
    ...
    def get_ris_visibility(self, ris_obj):
        return set(visible_vertices(Point(ris_obj.x,ris_obj.y), self.vis_graph.graph))
