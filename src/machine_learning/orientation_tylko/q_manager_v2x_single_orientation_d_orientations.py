...
d_orient_dict = {0: 0, 1: 1, 2: -1, 3: 0.5, 4: -0.5}
    ...
    def update_state_idxs(self, selected_pairs, dbs_coords):
        ...
        for idx, pair in enumerate(selected_pairs):
            if idx < qlp.N_Q_PAIRS:
                az_d_idx = self.get_az_idx_uniform(
                    (pair.stats.r_rx_phi % (2 * np.pi) - pair.stats.r_tx_phi % (2 * np.pi)) % (2 * np.pi))
                prev_az_d_idx = self.get_az_idx_uniform(
                    (pair.prev_stats.r_rx_phi % (2 * np.pi) - pair.prev_stats.r_tx_phi % (2 * np.pi)) % (2 * np.pi))
